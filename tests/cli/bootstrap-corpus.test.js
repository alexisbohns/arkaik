...
/**
 * Exercises `arkaik bootstrap` dispatch and `bootstrap corpus` — mining
 * merged PRs, design docs and code surfaces into .arkaik/corpus/ from a
 * captured gh payload or from git history directly.
 * Runs in fresh mkdtemp dirs, never the repo itself.
 *
 * Planning (`bootstrap plan`) and everything downstream of it lives in
 * tests/cli/bootstrap-plan.test.js — split out once this file passed 39
 * checks on its own, so Task 9's CI wiring only has to touch each file once.
 */
...
const dir = mkdtempSync(path.join(tmpdir(), "arkaik-bootstrap-corpus-"));
try {
