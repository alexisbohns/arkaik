...
| 5 (done) | arkaik | `arkaik kritik` verbs + nine `kritik_*` MCP tools over one shared implementation (`quality-ops.ts`, `cli/kritik-audit.ts`, `cli/kritik-overlay.ts`), so the scheduled agent audit becomes a monitoring loop |
| 6 | arkaik | Quality page + findings board reading the section; webhook grows `quality.finding.resolved`; the signal-pack runner |
| 7 | pbbls | Switch from sidecar-only to the first-class section; delete nothing (the journal already carries the history) |

## 8. Decisions and open questions

The first four were carried into the schema PR; three of them settled there and the fourth stays open by design. Three more were settled by the CLI/MCP build and are recorded below it.
...
Three more were settled while building the CLI and MCP surfaces (phase C):

5. **The verbs are stricter than the validator, on purpose.** An unknown criterion, an undeclared surface, and a score with no evidence all *fail* at the keystroke, where `validateBundle` only warns. The validator has to tolerate data it did not create; a command that is creating it does not, and the same typo caught two audits later costs the comparison rather than a retype.
6. **`signals` prints a run sheet; it does not execute one.** The pack's signals are statements to check — "grep X returns nothing", "a CI job asserting Y exists" — spanning greps, CI introspection and database queries. A runner that guessed at executing them would be wrong in ways nobody could see. So `arkaik kritik signals` emits the checks for the applicable cells and **exits 1 when anything has tripped since the last recorded audit**, which is the CI-usable half; the agent runs the checks and reports back with `--trip`. Regression detection *between* two audits' matrices stays with the phase-E runner. **Built in phase E** as `arkaik kritik regressions` / `kritik_regressions` / `detect-regressions.js`, over `detectRegressions` in `packages/schema/src/quality-regressions.ts`. Three kinds, all criterion-scoped because `quality.signal.tripped` requires a `criterion_id`: a maturity level that dropped, a cell that gained an open Critical or High, and a finding that was resolved and is open again. Cells scored in only one of the two audits are not compared — a half-finished audit is not a regression. `signals` itself is unchanged.
7. **The MCP mirror covers the audit loop, not its terms.** `kritik_profile` and `kritik_criterion_add` deliberately do not exist. Picking surfaces and authoring a criterion are one-time decisions about what this product is and how it should be judged — the terms an audit is measured against, not moves within one. They stay CLI and plugin actions, the same way `arkaik release` stays CLI-only.
8. **Hosted sessions read and transition findings; the audit run itself stays repo-only (issue #400).** The blanket "hosted has no code, so nothing here runs there" premise held only through phase 5 — as of the events-only write path it's half right. `kritik_matrix`, `kritik_findings` (fetchable by `finding_id`), `kritik_regressions` (assessment-level drops only — hosted findings are a living pool, not a per-audit snapshot, so the other two regression kinds don't compare) and `kritik_issue` read the quality section the server folds from that project's own journal (journal events applied), no checkout required. `kritik_resolve_finding` and `kritik_accept_finding` transition a finding by posting a whitelisted event — `quality.finding.resolved` or `quality.finding.accepted` — to `POST /api/graph/projects/{id}/quality/events` under `graph:write`; that route later took a third whitelisted type, `quality.signal.tripped` (issue #406), written not by an agent session but by a CI job, and admitted on a different ground: a trip decides nothing, so there is no verdict for it to overwrite and no finding for it to reach. The stored snapshot's `quality` section is never mutated by that route, and current status is re-derived on read, the same "current state is a projection" doctrine as §3.2 and the same path the GitHub App's own resolution pass already used. What stays repo-only is the audit *run* — `kritik_score`, `kritik_signals`, `kritik_trip_signal`, `kritik_open_finding` — because scoring a cell and opening a finding both read the code; those four now refuse with that reason stated, in place of the old undifferentiated hosted refusal. The surviving half of the original decision: running an audit needs a working tree, full stop — that was never about reading or deciding on what a past audit already found. Worth recording alongside it: the write is events-only, no snapshot mutation and no new tables; findings stay a living pool rather than gaining an `audit_id` partition in hosted mode; a finding's optional `commit` anchor is required by policy for any future hosted-written `kritik_open_finding`; the scope split is `graph:read` for the reads above and `graph:write` for the two transitions, plus — since #406 — an asymmetric third, `quality:append`, which admits only a trip, so a credential held in a **public** repository's Actions secrets can append an observation while being unable to read the graph or decide a finding (a finding decision from it is a 403 naming `graph:write`), and that trip must carry the `commit` it observed, required by the route rather than by the schema because CI is the one writer class that pays nothing for the anchor; the read side is unchanged by it — `kritik_signals` stays repo-only, because the signal pack and its run sheet live with the code, and the hosted trip is a write, not a window; and Publik's `stripQuality` is unaffected, since it removes the whole quality section before either path is reachable.
