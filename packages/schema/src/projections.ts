 ...
 * pattern as the app's status rollups (docs/spec/journal.md § Projections).
 * Module-local exported result types, minimal `Pick<>` inputs, no I/O, no React,
 * immutable: every function returns fresh data and never mutates its arguments.
 *
 * Lives in @arkaik/schema so the app (`lib/utils/journal.ts` re-exports these)
 * and the CLI (`arkaik log` / `arkaik release`) share one implementation instead
 * of duplicating the ordering and slicing rules. Deliberately **zod-free** —
 * only type-only imports from the schema package's own type modules plus the
 * runtime {@link orderEvents} helper — so it stays browser-safe and adds nothing
 * to the standalone validator bundle.
 *
 * The journal is the *read* surface here — these functions never emit events.
 * Ordering follows the journal ordering rule (by `ts`, tiebreaking by `id`);
 * {@link orderEvents} is reused so the rule lives in exactly one place. A bundle
 * without a journal yields the empty projection, never an error — that is the
 * whole backward-compatibility story (docs/spec/journal.md § Projections).
 */

import { orderEvents } from "./journal";
import type {
  JournalEvent,
  EdgeAddedEvent,
  ReleaseTaggedEvent,
