...
${p}`)}let c;if(r!==void 0){(0,m.existsSync)(r)||a(`init-profile: no config at ${r}`);let t=b(r);(!Array.isArray(t?.surfaces)||t.surfaces.length===0)&&a(`init-profile: ${r} declares no surfaces`),c=t}else{i.length===0&&a(`init-profile: at least one --surface is required

${p}`);let t=new Set;for(let s of i)t.has(s.id)&&a(`init-profile: duplicate surface id "${s.id}"`),t.add(s.id);c=Object.keys(n).length>0?{surfaces:i,domain_weights:n}:{surfaces:i}}let u=v(o);(0,m.existsSync)(u)&&!y&&a(`init-profile: ${u} already exists.
