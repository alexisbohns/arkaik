...
${g}`);let t=new Set;for(let o of i)t.has(o.id)&&a(`init-profile: duplicate surface id "${o.id}"`),t.add(o.id);c=Object.keys(n).length>0?{surfaces:i,domain_weights:n}:{surfaces:i}}let u=v(s);(0,m.existsSync)(u)&&!y&&a(`init-profile: ${u} already exists.
...
`+(c.domain_weights&&Object.keys(c.domain_weights).length>0?`weights: ${Object.entries(c.domain_weights).map(([t,o])=>`${t}=${o}`).join(" ")}
